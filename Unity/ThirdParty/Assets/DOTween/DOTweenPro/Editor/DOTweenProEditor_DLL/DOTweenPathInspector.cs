...
namespace DG.DOTweenEditor
{
	...
	public class DOTweenPathInspector : ABSAnimationInspector
	{
		...
		// (get) Token: 0x06000005 RID: 5 RVA: 0x000020BE File Offset: 0x000002BE
		...
		public override void OnInspectorGUI()
		{
			...
			if (this._miHasRigidbody == null)
			{
				...
				this._miHasRigidbody = looseScriptType.GetMethod("HasRigidbody", BindingFlags.Static | BindingFlags.Public);
			}
			...
			if (this._refreshAfterEnable)
			{
				...
				{
					Rect rect2 = new Rect(rect.xMin, rect.yMin, 23f, rect.height);
				    Rect rect3 = new Rect(rect2.xMax, rect2.yMin, rect.width - 23f, rect2.height);
					GUI.Label(rect2, (index + 1).ToString());
			...
			if (this._showTweenSettings)
			{
				...
				{
					GUILayout.BeginHorizontal(new GUILayoutOption[0]);
					bool flag2 = (bool)this._miHasRigidbody.Invoke(null, new object[]
					{
						this._src
					});
			...
			this._src.wpsDropdown = DeGUILayout.ToolbarFoldoutButton(this._src.wpsDropdown, "Waypoints", false, false, null);
		...
		// Token: 0x06000008 RID: 8 RVA: 0x00002D1C File Offset: 0x00000F1C
		private void OnSceneGUI()
		{
			...
			if (current.type == null)
			{
				if (current.shift)
				{
					if (EditorGUI.actionKey)
					{
						...
						float num = -(worldToCameraMatrix.m20 * vector.x + worldToCameraMatrix.m21 * vector.y + worldToCameraMatrix.m22 * vector.z + worldToCameraMatrix.m23);
						Vector3 item = this._sceneCam.ViewportToWorldPoint(new Vector3(current.mousePosition.x / this._sceneCam.pixelRect.width, 1f - current.mousePosition.y / this._sceneCam.pixelRect.height, num));
			...
			for (int i = 0; i < count; i++)
			{
				...
				float num2 = (this._src.handlesDrawMode == HandlesDrawMode.Orthographic) ? (HandleUtility.GetHandleSize(vector2) * 0.2f) : this._src.perspectiveHandleSize;
				bool flag3 = wpHandle.wpIndex >= 0 && wpHandle.wpIndex < (this._src.isClosedPath ? count : (count - 1));
				Vector3 vector3 = flag3 ? ((wpHandle.wpIndex >= count - 1) ? this._src.transform.position : this._src.wps[wpHandle.wpIndex + 1]) : Vector3.zero;
				bool flag4 = flag3 && Vector3.Distance(this._sceneCamTrans.position, vector2) < Vector3.Distance(this._sceneCamTrans.position, vector2 + Vector3.ClampMagnitude(vector3 - vector2, num2 * 1.75f));
				if (flag2)
				{
				...
				if (flag3 && flag4)
				{
					this.DrawArrowFor(wpHandle.wpIndex, num2, vector3);
				}
				...
				{
					vector2 = Handles.FreeMoveHandle(vector2, Quaternion.identity, num2, Vector3.one, new Handles.DrawCapFunction(Handles.SphereCap));
				}
				else
				{
				...
				Vector3 vector4 = this._sceneCamTrans.InverseTransformPoint(vector2) + new Vector3(num2 * 0.75f, 0.1f, 0f);
				vector4 = this._sceneCamTrans.TransformPoint(vector4);
				if (this._src.showIndexes || this._src.showWpLength)
				{
					...
					Handles.Label(vector4, text, flag2 ? EditorGUIUtils.handleSelectedLabelStyle : EditorGUIUtils.handlelabelStyle);
				}
			}
		...
		// Token: 0x06000009 RID: 9 RVA: 0x00003498 File Offset: 0x00001698
		...
		// Token: 0x0600000A RID: 10 RVA: 0x000034E8 File Offset: 0x000016E8
		...
		// Token: 0x0600000B RID: 11 RVA: 0x0000357C File Offset: 0x0000177C
		...
		// Token: 0x0600000C RID: 12 RVA: 0x000035E8 File Offset: 0x000017E8
		...
		// Token: 0x0600000D RID: 13 RVA: 0x000036D4 File Offset: 0x000018D4
		...
		// Token: 0x0600000E RID: 14 RVA: 0x00003738 File Offset: 0x00001938
		...
		// Token: 0x0600000F RID: 15 RVA: 0x000037A4 File Offset: 0x000019A4
		...
		// Token: 0x06000010 RID: 16 RVA: 0x0000389C File Offset: 0x00001A9C
		...
		// Token: 0x06000011 RID: 17 RVA: 0x0000392C File Offset: 0x00001B2C
		...
		private bool CheckTargetMove()
		{
			if (this._src.lastSrcPosition != this._src.transform.position)
			{
				if (this._src.relative)
				{
					...
					for (int i = 0; i < count; i++)
					{
						this._src.wps[i] = this._src.wps[i] + vector;
					}
				}
				this._src.lastSrcPosition = this._src.transform.position;
				return true;
		...
		// Token: 0x06000013 RID: 19 RVA: 0x00003A18 File Offset: 0x00001C18
		...
		// Token: 0x06000014 RID: 20 RVA: 0x00003AC4 File Offset: 0x00001CC4
		private void DropToFloor(float offsetY)
		{
			...
			for (int i = 0; i < this._src.wps.Count; i++)
			{
				...
				if (Physics.Raycast(vector, Vector3.down, out raycastHit, float.PositiveInfinity))
				{
					flag = true;
		...
		// Token: 0x06000015 RID: 21 RVA: 0x00003B7C File Offset: 0x00001D7C
